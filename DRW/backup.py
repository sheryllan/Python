...
def place_piece(piece, grid_columns):
    ...
    # find the row number of which the row is full
    div = full_row_mask / (2**y0)
    full_rows = []
    for i in range(piece_height):
        div, rem = divmod(div, 2)
        if rem:
            full_rows.append(y0 + i)

    if not full_rows:
        return

    # clear the full row bits in the grid_columns
    for i, col in enumerate(grid_columns):
        prev = -1
        new_col = 0
        full_rows.append(col.bit_length())
